...
def algoritmo_genetico(matriz_distancias, num_cidades, tamanho_populacao, num_geracoes, taxa_crossover, taxa_mutacao, taxa_infeccao):
    ...
    for geracao in range(num_geracoes):
        ...
        print(f"Geração {geracao + 1}: Melhor Custo = {melhor_fitness}")
...
if __name__ == "__main__":
    # *** 7. Execução Principal ***
    nome_arquivo_tsp = "nome_do_arquivo.tsp"  # Substitua pelo nome do seu arquivo TSPLIB
    ...
    if matriz_distancias is not None and num_cidades is not None:
        ...
        # *** 8. Visualização dos Resultados (Opcional) ***
        import matplotlib.pyplot as plt
        plt.plot(historico_custos)
        plt.xlabel("Geração")
        plt.ylabel("Melhor Custo")
        plt.title("Histórico do Melhor Custo por Geração")
        plt.grid(True)
        plt.show()
